...
\centerline{ \bf  M534H   HOMEWORK-- Spring 2020}
\vskip .1in
...
$\bullet$\, {\bf{Set 1. Due date: 02/06/2020 }}
\vskip .2in
...
$\bullet$\, {\bf{Set 2. Due date: 02/13/2020 }}

\vskip .1in

\underbar{Additional Problem}:   

a)  Find the general solution to Problem 8 in Section 1.2. 
...
$\bullet$\, {\bf{Set 3. Due date: 02/27/2020 }}

\vskip .1in
(Do not turn in problems that have an  $^\ast$ next to the number.)
...
\underbar{Extra Problem$^{*}$ }:  Prove the {\it Second Vanishing Theorem} in A.1. page 416

\vskip .2in 


\underbar{Section 1.4}:  Read Section 1.4.  Then do Problem 1.
%\smallskip 
%
...
\underbar{Additional Problem}:  \, Find the regions in $\Bbb R^2$  where $x^2\, u_{xx} + 4 u_{xy} +  y^2 \, u_{yy} \, =\,0$ 
is respectively elliptic, parabolic, hyperbolic. Plot these regions. 

\vskip .3in

$\bullet$\, {\bf{Set  4. Due date: 03/05/2020 }}
\vskip .1in 
\underbar{Section 2.1}: \quad 1, 2, 8,  9,  10, 11 
...
\vskip .2in
{\bf In the following 3 additional problems check first the second order PDE is hyperbolic}
\vskip .1in
...
\vskip .1in
\underbar{Additional Problem 5}: Find the solution the IVP  
...
$\bullet$\, {\bf{Set  5. Due date:  Monday March 30, 2020 (upload in Moodle as Single PDF document)}}


\vskip .1in 

\underbar{Additional Problem 1}  \, Consider the wave equation in $1$D with {\it damping} 
...
\underbar{Additional Problem 2:}  Let $u\,= \,u(\bold{x}, t)$ be a solution to the wave equation $u_{tt} - \Delta u =0$ in $\Bbb R^2$.  Assuming that $ \nabla u \to 0$ fast enough as $| \bold{x}| \to \infty$ prove that $$ E(t) \, =\, {\int}_{\Bbb R^2} \, \,  |u_t|^2 \, + \,  |\nabla\, u |^2 \, \, dx dy $$  is constant in $t$ for all time $t$. 
\vskip .2in
\underbar{Section 2.3}:\quad 2, \,4, \,5,  \,7a)$^{\ast}$. 
\vskip .1in 
...
$\bullet$\, {\bf{Set  6. Due date: Thursday 04/9/2020 }}
\vskip .2in

 \underbar{Section 2.4}: \quad   3,  4, 5a)b)c), 9, 11a), 15, 16.
\vskip .1in
{\underbar{Additional Problem} a) Show that the function $ u(x, t) = e^{-k t} \sin(x) $ solves the heat equation $ u_t - k u_{xx} = 0$.
\medskip
...
$\bullet$\, {\bf{Set  7. Due date: Thursday 04/23/2020 }}
...
\underbar{Additional Problem 1} Separate variables to solve the following problem for the {\it wave equation } with zero Neumann boundary conditions, 
$u_{tt} - 4 u_{xx} \,=\, 0$ in $0<x< L$ and  $u_x(0,t) = u_x (L,t)=0$

\vskip .1in

\underbar{Additional Problem 2} Separate variables to solve the following problem for the heat equation with zero Neumann boundary conditions, 
$u_{t} - 3 u_{xx} \,=\, 0$ in $0<x< \pi$ and $u_x(0,t) = u_x (\pi,t)=0$

\vskip .1in

\underbar{Read} first the last part of section 4.2 (page 91) where there is an example with mixed boundary conditions. \underbar{Then do}:  1, 2 in Section 4.2.



\vskip .3in

$\bullet$\, {\bf{Set  8. Due date: Wednesday 05/06/2020 }}


\vskip .2in

\underbar{Section 5.1} \, \, \,  2,  9 
\vskip .1in 
...
\underbar{Section 5.3} $3^{\dagger}$ 
\vskip .1in 
Problem $3^{\dagger}$ means that you should do this problem for {\bf zero Neumann boundary conditions instead of the mixed ones}.
That is,  consider the given wave equations with $u_x(0, t)=0= u_x(\ell, t)$ and the same initial data $u(x, 0)= x$ and 
$u_t(x, 0)=0$.
...
\underbar{Additional Problem$^{\ast}$ (do but do not turn in)} \,  Let $\phi : \Bbb R \to \Bbb R$ be a periodic function with period $p$; that is $\phi(x + p) = \phi(x), \, \, \forall x \in \Bbb R$. Assume that 
...
\underbar{\bf Special Project 2}  (a)  Solve the following hyperbolic initial value problem on $\Bbb R$ by first completing the square and solve the equation in terms of generic functions $f$ and $g$. Then use the initial conditions to choose appropriate $f, \, g$ and constants. 
