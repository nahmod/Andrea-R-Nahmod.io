...
\centerline{ \bf  M623 HOMEWORK  -- Fall 2024}
\vskip .1in
...
\head {\underbar{SET 1:  Due Date  09/19/2024}  }\endhead
...
\head {\underbar{SET 2:  Due Date 09/26/2024}  }\endhead

\medskip
 

\noindent {\bf From Chapter 1 (pp 37-42)}:\, 1, 2, 3.
...
{\bf A.I.} Construct a subset of $[0,1]$ in the same manner as the Cantor set, except that at the $k$th stage, each interval removed has length $\delta 3^{-k}$, for some $0<\delta<1$. Show that the resulting set is perfect, has measure $1-\delta$, and it is totally disconnected (in particular contains no intervals). 

\bigskip

 {\bf A.II.}   For $x \in [0,1]$,  let 
$$
...
{\bf A.III.} The following problem is a special case of Problem 4 in [SS, Ch1] dealing with what we call {\it Fat Cantor Sets}. 
...
{\bf A.IV.}   a) \, Let $ A = \cup_{n=1}^{\infty} A_n$ with $ m_{\ast}(A_n) = 0$. \underbar{Use} the definition of exterior measure to prove that $m_{\ast}(A) =0$.
...
\head {\underbar{SET 3:  Due Date 10/10/2024}  }\endhead

\bigskip

%Additional Problem from last time:
\smallskip
...
\medskip
\noindent {\bf From Chapter 1 (pp 37-42)}:  5, 6, 7, 10, 11, 16, 25. 
...
\head {\underbar{SET 4:  Due Date 10/17/2024}  }\endhead


\bigskip

\noindent {\bf From Chapter 1 (pp 37-42)}:  28, 29.


\medskip


{\bf A.I.}  Let $\{E_n\}_{n\geq 1}$ be a countable collection of measurable sets in $\Bbb R^d$. Define
...
$$  m( \limsup_{n\to \infty} E_n ) \geq   \limsup_{n\to \infty} m(E_n )  \quad \text{ provided that } \, m(\bigcup_{n=1}^\infty E_n ) < \infty $$
...
\smallskip
{\bf AII.}  \,   Do problem   13a)  \, in Chapter 1 page 41. 
...
\underbar{Additional Problem (do but do not turn in)}:  Read, and then write/type, explaining key points and expanding/filling gaps where necessary 
the construction of a {\it nonmeasurable set} in Stein-Shakarchi's book in pages 24-25-26.   Note in the course of the proof you might need to use the Invariance properties of the 
Lebesgue measure in the bottom half of page 22. 


{\bf AIII.}  First do  32 (pp 44-45).  (Hint for part a) consider the sets $E_k = E + r_k \subset \Cal N_k$, where $\{r_k\}_{k \geq 1}$ is an enumeration of the rationals.)
...
\underbar{Additional Problem (do but do not turn in)}:  {From Chapter 1 (pp 37-42)}  do problem 10 
...
\head {\underbar{SET 5:  Due Date 10/24/2024}  }\endhead
\vskip .15in
...
\noindent {\bf From Chapter 1 (pp 37-42)}:   Prove  problem $18$  in this case only  ``Every measurable function $f: [a, b]  \to \Bbb R$  (finite-valued ) is the limit a.e. of a sequence of continuous functions on $[a,b]$."   

\medskip

\underbar{Hint} This is Lusin's Theorem. Note the Handout and fully rewrite on your own thinking the proof in this case.

\vskip .15in

\noindent{\bf  Additional Problem AI.}  The following relates to the proof of Theorem 4.1  page  31. Prove that the sequence of nonnegative simple functions $\{\phi_k\}_k$ that 
...
%\head {\underbar{SET 5:  Due Date 10/19/2023}  }\endhead
%\
...
%
%
%{\bf A.II.}  Let $\{E_n\}_{n\geq 1}$ be a countable collection of measurable sets in $\Bbb R^d$. Define
%$$  \quad  \limsup_{n \to \infty} E_n \,:=\, \{ x \in \Bbb R^d \, :\, \, x \in E_n, \, \text{ for infinitely many } \, n \, \} $$
%$$  \quad  \liminf_{n \to \infty} E_n \,:=\, \{ x \in \Bbb R^d \, :\, \, x \in E_n, \, \text{ for all but finitely many } \, n \, \} $$
% 
%\smallskip 
...
%\vskip .15in
%
%{\bf AIII.}  \,   Do problem   13a)  \, in Chapter 1 page 41. 
%
%\medskip
%{\underbar{Hints for 13a)} First show that for each $n \in \Bbb N$, the set $\Cal O_n:= \{ x \, : \, d(x, F) < \frac{1}{n} \}$ is open. 
%
...
%\head {\underbar{SET 6:  Due Date 11/07/2024}  }\endhead
%\vskip .15in
%
%\noindent {\bf From Chapter 1 (pp 37-42)}:\  17,  22 
%\smallskip
...
%\bigskip
%
%\noindent {\bf From Chapter 1 (pp 37-42)}:   Prove  problem $18$  in this case only  ``Every measurable function $f: [a, b]  \to \Bbb R$  is the limit a.e. of a sequence of continuous functions on $[a,b]$."   
%
%
%\vskip .15in
%
...
\head {\underbar{SET 6:  Due Date 11/14/2024}  }\endhead
\vskip .15in
...
\smallskip
{\bf A.I.\,} If a function $f$ is integrable then we proved in Proposition 1.12 (Chapter 2) that for any $\varepsilon>0$ there exists a $\delta >0$
...
%\medskip
%
%{\bf II.} We say that a sequence of measurable functions $\{f_n\}_{n\geq 1}$ {\it converges in measure} to another measurable function $f$ is for every $\varepsilon >0$, 
%$$ m(\{ x \,:\, |f_n(x) \,- \, f(x)| > \varepsilon \} ) \, \to \, 0, \qquad \text{as} \qquad n \to \infty $$
%Prove that if a sequence of measurable function $f_n$ converges in measure to another measurable function $f$ then there exists a subsequence $\{f_{n_j}\}_{j \geq 1}$ which converges almost everywhere to $f$, that is $ f_{n_j}(x) \to f(x)$ a.e. $x$ as $ j \to \infty$.
%\smallskip
%\underbar{Hint.} First  show that for $\varepsilon = 2^{-j}$ one can choose $n_j$ such that for all $n \geq n_j$ 
...
%\medskip
%
%{\bf III.} Suppose that $\{f_n\}_{n \geq 1}$ is a sequence of non-negative measurable function, that is $f_n\geq 0$ for all $n$, such that $ f_n$ converges in measure to $f$ . Show that then 
%
%$$ \int f(x) \, \, dm \, \leq \, \liminf_n \, \int f_n(x)\, \, dm $$
%
%\smallskip
...
\vskip .3in
\head {\underbar{SET 7:  11/21/2024.}  }\endhead


\bigskip
\noindent {\bf From Chapter 2 (pp 89-93)}: 12, 15, 16.

\smallskip
{\underbar{Hint} Recall the counter-examples we did in class.}
...
\head {\underbar{SET 8:  Due Date 12/05/2024}  }\endhead

\bigskip

\noindent {\bf Chapter 2 (pp 90-95)}:  2, 4, 18, 19, 22
...
{\bf II.\,}  {\bf Read  carefully the Handout:} {\it{On Fubini and Product Sets}} in the class webpage under `Some useful notes'.

{\bf III.\,}  Let $f(x)$ be a measurable function over $\Bbb R^{d_1}$ and $g(y)$  be a measurable function over $\Bbb R^{d_2}$. Prove that $F(x,y) = f(x) g(y) $ is a measurable function over $\Bbb R^{d_1 + d_2}$.
\smallskip

\underbar{Hint.} The Handout, that you had to read in {\bf II.} above might be useful.

\medskip
{\bf IV.\,} Let $s$ be a fixed positive number.  Prove that
$$
...
\medskip
{\bf V.\,}   Consider the function $f(x,y):=  e^{-xy} - 2 e^{-2xy}$ where $ x \in [ 0, \infty)$ and $ y \in  [ 0,1]$. 
...
\noindent {\bf From Chapter  3 (pp 145-147)}: \, 1, 2, 4, 5, 7. 
...
\noindent {\bf Chapter 2 (pp 90-95)}:   4, 18, 19
...
\bigskip
\noindent {\bf From Chapter  3 (pp 145-147)}: \, 4, 5, 7, 11, 12, 15, 16a)
